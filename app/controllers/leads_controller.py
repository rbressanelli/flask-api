...
from app.services.auxiliar_functions import (
    date_maker, 
    phone_check, 
    keys_check, 
...
def create_new_lead():
    ...
    try:
        ...
        if type(err.__dict__['orig']).__name__ == 'UniqueViolation':
            return jsonify({
                    "error": "Lead informed already registered."
                }), HTTPStatus.CONFLICT
        elif type(err.__dict__['orig']).__name__ == 'NotNullViolation':
            return jsonify({
                "error": "All fields shound be foward to server."
            }), HTTPStatus.BAD_REQUEST
            
    except BadRequest:
        return jsonify({
            'error': 'Phone number format should be (XX)XXXXX-XXXX'
        }), HTTPStatus.BAD_REQUEST
...
def get_all_leads():
    ...
    leads = LeadsModel.query.order_by(LeadsModel.last_visit, )
